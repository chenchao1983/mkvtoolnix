...
generic_packetizer_c::set_video_colour_matrix(int matrix_index,
  ...
  if (m_track_entry && (matrix_index >= 0) && (matrix_index <= 10)) {
...
generic_packetizer_c::set_video_cb_subsample(const cb_subsample_t &subsample,
  ...
  if (m_track_entry && ((subsample.hori >= 0) || (subsample.vert >= 0))) {
    auto &video = GetChild<KaxTrackVideo>(m_track_entry);
    auto &color = GetChild<KaxVideoColour>(video);
    if (subsample.hori >= 0)
...
generic_packetizer_c::set_video_chroma_coordinates(chroma_coordinates_t const &coordinates,
                                                   option_source_e source) {
  m_ti.m_chroma_coordinates.set(chroma_coordinates_t(
      coordinates.red_x, coordinates.red_y, coordinates.green_x,
      coordinates.green_y, coordinates.blue_x,coordinates.blue_y), source);
  if (m_track_entry) {
    auto &video = GetChild<KaxTrackVideo>(m_track_entry);
    auto &color = GetChild<KaxVideoColour>(video);
...
generic_packetizer_c::set_video_max_luminance(float luminance,
  ...
  if (m_track_entry && (luminance >= 0) && (luminance <= 9999.99)) {
    auto &video = GetChild<KaxTrackVideo>(m_track_entry);
    auto &color = GetChild<KaxVideoColour>(video);
  ...
  if (track_video == m_htrack_type) {
    ...
    if ((-1 != m_hvideo_pixel_height) && (-1 != m_hvideo_pixel_width)) {
      ...
      if (m_ti.m_max_luminance) {
        ...
        auto &colour   = GetChild<KaxVideoColour>(video);
        KaxVideoColourMasterMeta& master_meta = GetChild<KaxVideoColourMasterMeta>(colour);
