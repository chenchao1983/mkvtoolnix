...
class atom_chunk_size_x : public exception {
 ...
 public:
  atom_chunk_size_x(std::size_t size, std::size_t pos)
      : mtx::exception(), m_size{size}, m_pos{pos} {}

  virtual const char *what() const throw() { return "invalid atom chunk size"; }

  virtual std::string error() const throw() {
    return (boost::format("invalid atom chunk size %1% at %2%") % m_size %
            m_pos)
        .str();
  }
};
...
static std::string space(int num) { return std::string(num, ' '); }

static qt_atom_t read_qtmp4_atom(mm_io_c *read_from,
  ...
  if (a.size < a.hsize) {
    if (exit_on_error)
      mxerror(boost::format(
                  Y("Quicktime/MP4 reader: Invalid chunk size %1% at %2%.\n")) %
              a.size % a.pos);
    else
...
int qtmp4_reader_c::probe_file(mm_io_c *in, uint64_t) {
  try {
    ...
    while (1) {
      ...
      if (1 == atom_size) atom_size = in->read_uint64_be();

      mxverb(3, boost::format("Quicktime/MP4 reader: Atom: '%1%': %2%\n") %
                    atom % atom_size);

      if ((atom == "moov") || (atom == "ftyp") || (atom == "mdat") ||
          (atom == "pnot"))
        return 1;

      if ((atom != "wide") && (atom != "skip")) return 0;
...
qtmp4_reader_c::qtmp4_reader_c(const track_info_c &ti, const mm_io_cptr &in)
...
void qtmp4_reader_c::read_headers() {
  try {
...
qt_atom_t qtmp4_reader_c::read_atom(mm_io_c *read_from, bool exit_on_error) {
  return read_qtmp4_atom(read_from ? read_from : m_in.get(), exit_on_error);
}

#define skip_atom() m_in->setFilePointer(atom.pos + atom.size)

bool qtmp4_reader_c::resync_to_top_level_atom(uint64_t start_pos) {
  static std::vector<std::string> const s_top_level_atoms{
      "ftyp", "pdin", "moov", "moof", "mfra", "mdat", "free", "skip"};
  static auto test_atom_at = [this](uint64_t atom_pos, uint64_t expected_hsize,
                                    fourcc_c const &expected_fourcc) -> bool {
    m_in->setFilePointer(atom_pos);
    auto test_atom = read_atom(nullptr, false);
    mxdebug_if(m_debug_resync,
               boost::format("Test for %1%bit offset atom: %2%\n") %
                   (8 == expected_hsize ? 32 : 64) % test_atom);

    if ((test_atom.fourcc == expected_fourcc) &&
        (test_atom.hsize == expected_hsize) &&
        ((test_atom.pos + test_atom.size) <= m_size)) {
      mxdebug_if(m_debug_resync,
                 boost::format("%1%bit offset atom looks good\n") %
  ...
  try {
    ...
    mxdebug_if(m_debug_resync,
               boost::format("Starting resync at %1%, FourCC %2%\n") %
                   start_pos % fourcc);
    while (true) {
      ...
      if (test_atom_at(fourcc_pos - 12, 16, fourcc) ||
          test_atom_at(fourcc_pos - 4, 8, fourcc))
        return true;
    }

  } catch (mtx::mm_io::exception &ex) {
    mxdebug_if(m_debug_resync,
               boost::format("I/O exception during resync: %1%\n") % ex.what());
  } catch (mtx::atom_chunk_size_x &ex) {
    mxdebug_if(
        m_debug_resync,
        boost::format("Atom exception during resync: %1%\n") % ex.error());
  }
...
void qtmp4_reader_c::parse_headers() {
  ...
  try {
    while (true) {
      ...
      mxdebug_if(
          m_debug_headers,
          boost::format(
              "'%1%' atom, size %2%, at %3%–%4%, human readable? %5%\n") %
              atom.fourcc % atom.size % atom.pos % (atom.pos + atom.size) %
              atom.fourcc.human_readable());

      if (atom.fourcc == "ftyp") {
        auto tmp = fourcc_c{m_in};
        mxdebug_if(m_debug_headers,
                   boost::format("  File type major brand: %1%\n") % tmp);
        tmp = fourcc_c{m_in};
        mxdebug_if(m_debug_headers,
                   boost::format("  File type minor brand: %1%\n") % tmp);

        for (idx = 0; idx < ((atom.size - 16) / 4); ++idx) {
          tmp = fourcc_c{m_in};
          mxdebug_if(
              m_debug_headers,
              boost::format("  File type compatible brands #%1%: %2%\n") % idx %
                  tmp);
        }
      ...
      if (headers_parsed && mdat_found && !moof_found) break;
    }
  } catch (mtx::mm_io::exception &) {
  }
  ...
  if (!mdat_found)
    mxerror(
        Y("Quicktime/MP4 reader: Have not found the 'mdat' atom. No movie data "
          "found.\n"));
  ...
  brng::remove_erase_if(m_demuxers, [this](qtmp4_demuxer_cptr const &dmx) {
    return !dmx->ok || dmx->is_chapters();
  });

  detect_interleaving();

  if (!g_identifying) calculate_timecodes();

  mxdebug_if(
      m_debug_headers,
      boost::format("Number of valid tracks found: %1%\n") % m_demuxers.size());
}

void qtmp4_reader_c::verify_track_parameters_and_update_indexes() {
...
void qtmp4_reader_c::calculate_timecodes() {
  if (m_timecodes_calculated) return;
  ...
  if (0 > min_timecode)
    for (auto &dmx : m_demuxers) dmx->adjust_timecodes(-min_timecode);

  m_timecodes_calculated = true;
}

void qtmp4_reader_c::handle_audio_encoder_delay(qtmp4_demuxer_cptr &dmx) {
  if ((0 == m_audio_encoder_delay_samples) || (0 == dmx->a_samplerate) ||
      (-1 == dmx->ptzr))
    return;

  PTZR(dmx->ptzr)->m_ti.m_tcsync.displacement -=
      (m_audio_encoder_delay_samples * 1000000000ll) / dmx->a_samplerate;
  m_audio_encoder_delay_samples = 0;
}

#define print_basic_atom_info()                                              \
  mxdebug_if(m_debug_headers,                                                \
             boost::format("%1%'%2%' atom, size %3%, at %4%–%5%\n") %        \
                 space(2 * level + 1) % atom.fourcc % atom.size % atom.pos % \
                 (atom.pos + atom.size));

#define print_atom_too_small_error(name, type)                              \
  mxerror(boost::format(Y("Quicktime/MP4 reader: '%1%' atom is too small. " \
                          "Expected size: >= %2%. Actual size: %3%.\n")) %  \
          name % sizeof(type) % atom.size);

void qtmp4_reader_c::handle_cmov_atom(qt_atom_t parent, int level) {
...
void qtmp4_reader_c::handle_cmvd_atom(qt_atom_t atom, int level) {
  ...
  if (m_compression_algorithm != "zlib")
    mxerror(boost::format(
                Y("Quicktime/MP4 reader: This file uses compressed headers "
                  "with an unknown "
  ...
  if (Z_OK != zret)
    mxerror(boost::format(
                Y("Quicktime/MP4 reader: This file uses compressed headers, "
                  "but the zlib library could not be initialized. "
                  "Error code from zlib: %1%. Aborting.\n")) %
            zret);

  zret = inflate(&zs, Z_NO_FLUSH);
  if ((Z_OK != zret) && (Z_STREAM_END != zret))
    mxerror(boost::format(Y("Quicktime/MP4 reader: This file uses compressed "
                            "headers, but they could not be uncompressed. "
                            "Error code from zlib: %1%. Aborting.\n")) %
            zret);

  if (moov_size != zs.total_out)
    mxwarn(boost::format(
               Y("Quicktime/MP4 reader: This file uses compressed headers, but "
                 "the expected uncompressed size (%1%) "
  ...
  while (!m_in->eof()) {
    ...
    mxdebug_if(m_debug_headers, boost::format("%1%'%2%' atom at %3%\n") %
                                    space((level + 1) * 2 + 1) %
                                    next_atom.fourcc % next_atom.pos);
...
void qtmp4_reader_c::handle_ctts_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  m_in->skip(1 + 3);  // version & flags
  uint32_t count = m_in->read_uint32_be();
  mxdebug_if(m_debug_headers,
             boost::format("%1%Frame offset table: %2% raw entries\n") %
  ...
  if (m_debug_tables) {
    ...
    for (auto const &frame_offset : new_dmx->raw_frame_offset_table)
      mxdebug(boost::format("%1%%2%: count %3% offset %4%\n") %
              space((level + 1) * 2 + 1) % i++ % frame_offset.count %
              frame_offset.offset);
  }
}

void qtmp4_reader_c::handle_sgpd_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  auto version = m_in->read_uint8();
  m_in->skip(3);  // flags

  auto grouping_type = fourcc_c{m_in};
  auto default_description_length =
      version == 1 ? m_in->read_uint32_be() : uint32_t{};

  if (version >= 2) m_in->skip(4);  // default_sample_description_index

  auto count = m_in->read_uint32_be();

  mxdebug_if(m_debug_headers,
             boost::format("%1%Sample group description table: version %2%, "
                           "type '%3%', %4% raw entries\n") %
                 space(level * 2 + 2) % static_cast<unsigned int>(version) %
                 grouping_type % count);

  if (grouping_type != "rap ") return;
  ...
  for (auto idx = 0u; idx < count; ++idx) {
    ...
    new_dmx->random_access_point_table.emplace_back(num_leading_samples_known,
                                                    num_leading_samples);
  }
  ...
  if (m_debug_tables) {
    ...
    for (auto const &rap : new_dmx->random_access_point_table)
      mxdebug(boost::format("%1%%2%: leading samples known %3% num %4%\n") %
              space((level + 1) * 2 + 2) % idx++ %
              rap.num_leading_samples_known % rap.num_leading_samples);
  }
}

void qtmp4_reader_c::handle_sbgp_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  auto version = m_in->read_uint8();
  m_in->skip(3);  // flags
  ...
  if (version == 1) m_in->skip(4);  // grouping_type_parameter

  auto count = m_in->read_uint32_be();

  mxdebug_if(m_debug_headers,
             boost::format("%1%Sample to group table: version %2%, type '%3%', "
                           "%4% raw entries\n") %
                 space(level * 2 + 2) % static_cast<unsigned int>(version) %
                 grouping_type % count);
  ...
  if (m_debug_tables) {
    ...
    for (auto const &s2g : table)
      mxdebug(
          boost::format("%1%%2%: sample count %3% group description %4%\n") %
          space((level + 1) * 2 + 2) % idx++ % s2g.sample_count %
          s2g.group_description_index);
  }
}

void qtmp4_reader_c::handle_dcom_atom(qt_atom_t, int level) {
...
void qtmp4_reader_c::handle_hdlr_atom(qtmp4_demuxer_cptr &new_dmx,
                                      qt_atom_t atom, int level) {
  hdlr_atom_t hdlr;
  ...
  mxdebug_if(m_debug_headers,
             boost::format("%1%Component type: %|2$.4s| subtype: %|3$.4s|\n") %
                 space(level * 2 + 1) % (char *)&hdlr.type %
                 (char *)&hdlr.subtype);
...
void qtmp4_reader_c::handle_mdhd_atom(qtmp4_demuxer_cptr &new_dmx,
                                      qt_atom_t atom, int level) {
  if (1 > atom.size) print_atom_too_small_error("mdhd", mdhd_atom_t);
  ...
  if (0 == version) {
    ...
    if (m_in->read(&mdhd.flags, sizeof(mdhd_atom_t) - 1) !=
    ...
    new_dmx->language =
        decode_and_verify_language(get_uint16_be(&mdhd.language));
    ...
    if (m_in->read(&mdhd.flags, sizeof(mdhd64_atom_t) - 1) !=
    ...
    mxerror(boost::format(Y("Quicktime/MP4 reader: The 'media header' atom "
                            "('mdhd') uses the unsupported version %1%.\n")) %
            version);

  mxdebug_if(
      m_debug_headers,
      boost::format("%1%Time scale: %2%, duration: %3%, language: %4%\n") %
          space(level * 2 + 1) % new_dmx->time_scale %
          new_dmx->global_duration % new_dmx->language);

  if (0 == new_dmx->time_scale)
    mxerror(
        Y("Quicktime/MP4 reader: The 'time scale' parameter was 0. This is not "
          "supported.\n"));
}

void qtmp4_reader_c::handle_mdia_atom(qtmp4_demuxer_cptr &new_dmx,
                                      qt_atom_t parent, int level) {
  while (parent.size > 0) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();
...
void qtmp4_reader_c::handle_minf_atom(qtmp4_demuxer_cptr &new_dmx,
                                      qt_atom_t parent, int level) {
  while (0 < parent.size) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();
...
void qtmp4_reader_c::handle_moov_atom(qt_atom_t parent, int level) {
  while (parent.size > 0) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();
    ...
    if (atom.fourcc == "cmov")
      ...
      if ((!new_dmx->is_unknown() && new_dmx->codec) ||
          new_dmx->is_subtitles() || new_dmx->is_video() || new_dmx->is_audio())
...
void qtmp4_reader_c::handle_mvex_atom(qt_atom_t parent, int level) {
  while (8 <= parent.size) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();

    if (atom.fourcc == "trex") handle_trex_atom(atom.to_parent(), level + 1);
...
void qtmp4_reader_c::handle_trex_atom(qt_atom_t, int level) {
  m_in->skip(1 + 3);  // Version, flags
  ...
  mxdebug_if(m_debug_headers,
             boost::format("%1%Sample defaults for track ID %2%: description "
                           "idx %3% duration %4% size %5% flags %6%\n") %
                 space(level * 2 + 1) % track_id %
                 defaults.sample_description_id % defaults.sample_duration %
                 defaults.sample_size % defaults.sample_flags);
}

void qtmp4_reader_c::handle_moof_atom(qt_atom_t parent, int level,
  ...
  while (8 <= parent.size) {
    ...
    if (atom.fourcc == "traf") handle_traf_atom(atom.to_parent(), level + 1);
...
void qtmp4_reader_c::handle_traf_atom(qt_atom_t parent, int level) {
  while (8 <= parent.size) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();
...
void qtmp4_reader_c::handle_tfhd_atom(qt_atom_t, int level) {
  m_in->skip(1);  // Version

  auto flags = m_in->read_uint24_be();
  auto track_id = m_in->read_uint32_be();
  auto track_itr = brng::find_if(
      m_demuxers, [this, track_id](qtmp4_demuxer_cptr const &dmx) {
        return dmx->container_id == track_id;
      });

  if (!track_id || !mtx::includes(m_track_defaults, track_id) ||
      (m_demuxers.end() == track_itr)) {
    mxdebug_if(
        m_debug_headers,
        boost::format("%1%tfhd atom with track_id(%2%) == 0, no entry in trex "
                      "for it or no track(%3%) found\n") %
            space(level * 2 + 1) % track_id %
            (m_demuxers.end() == track_itr ? nullptr : track_itr->get()));
    return;
  }
  ...
  fragment.base_data_offset = flags & QTMP4_TFHD_BASE_DATA_OFFSET
                                  ? m_in->read_uint64_be()
                                  : flags & QTMP4_TFHD_DEFAULT_BASE_IS_MOOF
                                        ? fragment.moof_offset
                                        : fragment.implicit_offset;
  fragment.sample_description_id = flags & QTMP4_TFHD_SAMPLE_DESCRIPTION_ID
                                       ? m_in->read_uint32_be()
                                       : defaults.sample_description_id;
  fragment.sample_duration = flags & QTMP4_TFHD_DEFAULT_DURATION
                                 ? m_in->read_uint32_be()
                                 : defaults.sample_duration;
  fragment.sample_size = flags & QTMP4_TFHD_DEFAULT_SIZE
                             ? m_in->read_uint32_be()
                             : defaults.sample_size;
  fragment.sample_flags = flags & QTMP4_TFHD_DEFAULT_FLAGS
                              ? m_in->read_uint32_be()
  ...
  mxdebug_if(m_debug_headers,
             boost::format("%1%Atom flags 0x%|2$05x| track_id %3% fragment "
                           "content: moof_off %4% implicit_off %5% "
                           "base_data_off %6% stsd_id %7% sample_duration %8% "
                           "sample_size %9% sample_flags %10%\n") %
                 space(level * 2 + 1) % flags % track_id %
                 fragment.moof_offset % fragment.implicit_offset %
                 fragment.base_data_offset % fragment.sample_description_id %
                 fragment.sample_duration % fragment.sample_size %
                 fragment.sample_flags);
}

void qtmp4_reader_c::handle_trun_atom(qt_atom_t, int level) {
  if (!m_fragment || !m_track_for_fragment) {
    mxdebug_if(
        m_debug_headers,
        boost::format(
            "%1%No current fragment (%2%) or track for that fragment (%3%)\n") %
  ...
  auto data_offset =
      flags & QTMP4_TRUN_DATA_OFFSET ? m_in->read_uint32_be() : 0;
  auto first_sample_flags = flags & QTMP4_TRUN_FIRST_SAMPLE_FLAGS
                                ? m_in->read_uint32_be()
                                : m_fragment->sample_flags;
  auto offset = m_fragment->base_data_offset + data_offset;

  for (auto idx = 0u; idx < entries; ++idx) {
    auto sample_duration = flags & QTMP4_TRUN_SAMPLE_DURATION
                               ? m_in->read_uint32_be()
                               : m_fragment->sample_duration;
    auto sample_size = flags & QTMP4_TRUN_SAMPLE_SIZE ? m_in->read_uint32_be()
                                                      : m_fragment->sample_size;
    auto sample_flags =
        flags & QTMP4_TRUN_SAMPLE_FLAGS
            ? m_in->read_uint32_be()
            : idx > 0 ? m_fragment->sample_flags : first_sample_flags;
    auto ctts_duration =
        flags & QTMP4_TRUN_SAMPLE_CTS_OFFSET ? m_in->read_uint32_be() : 0;
    auto keyframe =
        !track.is_video()
            ? true
            : !(sample_flags & (QTMP4_FRAG_SAMPLE_FLAG_IS_NON_SYNC |
                                QTMP4_FRAG_SAMPLE_FLAG_DEPENDS_YES));
  ...
  if (m_debug_tables) {
    ...
    for (auto idx = 0u; idx < entries; ++idx)
      mxdebug(boost::format("%1%%2%: duration %3% size %4% data start %5% end "
                            "%6% pts offset %7%\n") %
              spc % idx % track.durmap_table[durmap_start + idx].duration %
...
void qtmp4_reader_c::handle_mvhd_atom(qt_atom_t atom, int level) {
  mvhd_atom_t mvhd;
...
void qtmp4_reader_c::handle_udta_atom(qt_atom_t parent, int level) {
  while (8 <= parent.size) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();
...
void qtmp4_reader_c::handle_chpl_atom(qt_atom_t, int level) {
  if (m_ti.m_no_chapters || m_chapters) return;
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (m_in->read(buf->get_buffer(), buf->get_size() - 1) !=
        (buf->get_size() - 1))
      break;

    entries.push_back(qtmp4_chapter_entry_t(
        std::string(reinterpret_cast<char *>(buf->get_buffer())), timecode));
  }
...
void qtmp4_reader_c::handle_meta_atom(qt_atom_t parent, int level) {
  m_in->skip(1 + 3);  // version & flags
  ...
  while (8 <= parent.size) {
    ...
    if (atom.fourcc == "ilst") handle_ilst_atom(atom.to_parent(), level + 1);
...
void qtmp4_reader_c::handle_ilst_atom(qt_atom_t parent, int level) {
  while (8 <= parent.size) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();

    if (atom.fourcc == "----") handle_4dashes_atom(atom.to_parent(), level + 1);
...
std::string qtmp4_reader_c::read_string_atom(qt_atom_t atom,
                                             size_t num_skipped) {
  if ((num_skipped + atom.hsize) > atom.size) return "";
...
void qtmp4_reader_c::handle_4dashes_atom(qt_atom_t parent, int level) {
  ...
  mxdebug_if(m_debug_headers,
             boost::format("'----' content: name=%1% mean=%2% data=%3%\n") %
                 name % mean % data);

  if (name == "iTunSMPB") parse_itunsmpb(data);
}

void qtmp4_reader_c::parse_itunsmpb(std::string data) {
  data = boost::regex_replace(
      data, boost::regex("[^\\da-fA-F]+", boost::regex::perl), "");

  if (16 > data.length()) return;
...
void qtmp4_reader_c::read_chapter_track() {
  if (m_ti.m_no_chapters || m_chapters) return;

  auto chapter_dmx_itr = brng::find_if(
      m_demuxers,
      [this](qtmp4_demuxer_cptr const &dmx) { return dmx->is_chapters(); });
  if (m_demuxers.end() == chapter_dmx_itr) return;

  if ((*chapter_dmx_itr)->sample_table.empty()) return;

  std::vector<qtmp4_chapter_entry_t> entries;
  uint64_t pts_scale_gcd =
      boost::math::gcd(static_cast<uint64_t>(1000000000ull),
                       static_cast<uint64_t>((*chapter_dmx_itr)->time_scale));
  uint64_t pts_scale_num = 1000000000ull / pts_scale_gcd;
  uint64_t pts_scale_den =
      static_cast<uint64_t>((*chapter_dmx_itr)->time_scale) / pts_scale_gcd;

  for (auto &sample : (*chapter_dmx_itr)->sample_table) {
    if (2 >= sample.size) continue;

    m_in->setFilePointer(sample.pos, seek_beginning);
    memory_cptr chunk(memory_c::alloc(sample.size));
    if (m_in->read(chunk->get_buffer(), sample.size) != sample.size) continue;

    unsigned int name_len = get_uint16_be(chunk->get_buffer());
    if ((name_len + 2) > sample.size) continue;

    entries.push_back(qtmp4_chapter_entry_t(
        std::string(reinterpret_cast<char *>(chunk->get_buffer()) + 2,
                    name_len),
...
void qtmp4_reader_c::process_chapter_entries(
    int level, std::vector<qtmp4_chapter_entry_t> &entries) {
  ...
  for (; entries.size() > i; ++i) {
    ...
    mxdebug_if(m_debug_chapters, boost::format("%1%%2%: start %4% name %3%\n") %
                                     space((level + 1) * 2 + 1) % i %
                                     chapter.m_name %
                                     format_timestamp(chapter.m_timecode));

    out.puts(
        boost::format("CHAPTER%|1$02d|=%|2$02d|:%|3$02d|:%|4$02d|.%|5$03d|\n"
                      "CHAPTER%|1$02d|NAME=%6%\n") %
        i % (chapter.m_timecode / 60 / 60 / 1000000000) %
        ((chapter.m_timecode / 60 / 1000000000) % 60) %
        ((chapter.m_timecode / 1000000000) % 60) %
        ((chapter.m_timecode / 1000000) % 1000) % chapter.m_name);
  }

  mm_text_io_c text_out(&out, false);
  try {
    m_chapters =
        parse_chapters(&text_out, 0, -1, 0, m_ti.m_chapter_language, "", true);
    align_chapter_edition_uids(m_chapters.get());
  } catch (mtx::chapter_parser_x &ex) {
    mxerror(boost::format(
                Y("The MP4 file '%1%' contains chapters whose format was not "
                  "recognized. This is often the case if the chapters are not "
                  "encoded in UTF-8. Use the '--chapter-charset' option in "
                  "order to specify the charset to use.\n")) %
            m_ti.m_fname);
  }
}

void qtmp4_reader_c::handle_stbl_atom(qtmp4_demuxer_cptr &new_dmx,
                                      qt_atom_t parent, int level) {
  while (0 < parent.size) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();
...
void qtmp4_reader_c::handle_stco_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  m_in->skip(1 + 3);  // version & flags
  uint32_t count = m_in->read_uint32_be();

  mxdebug_if(m_debug_headers,
             boost::format("%1%Chunk offset table: %2% entries\n") %
                 space(level * 2 + 1) % count);
...
void qtmp4_reader_c::handle_co64_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  m_in->skip(1 + 3);  // version & flags
  uint32_t count = m_in->read_uint32_be();

  mxdebug_if(m_debug_headers,
             boost::format("%1%64bit chunk offset table: %2% entries\n") %
                 space(level * 2 + 1) % count);
...
void qtmp4_reader_c::handle_stsc_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  m_in->skip(1 + 3);  // version & flags
  uint32_t count = m_in->read_uint32_be();
  size_t i;
  ...
  mxdebug_if(m_debug_headers,
             boost::format("%1%Sample to chunk/chunkmap table: %2% entries\n") %
                 space(level * 2 + 1) % count);
  if (m_debug_tables) {
    i = 0;
    for (auto const &chunkmap : new_dmx->chunkmap_table)
      mxdebug(boost::format("%1%%2%: first_chunk %3% samples_per_chunk %4% "
                            "sample_description_id %5%\n") %
              space((level + 1) * 2 + 1) % i++ % chunkmap.first_chunk %
              chunkmap.samples_per_chunk % chunkmap.sample_description_id);
  }
}

void qtmp4_reader_c::handle_stsd_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  m_in->skip(1 + 3);  // version & flags
  uint32_t count = m_in->read_uint32_be();
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (4 > size)
      mxerror(boost::format(
                  Y("Quicktime/MP4 reader: The 'size' field is too small in "
                    "the stream description atom for track ID %1%.\n")) %
              new_dmx->id);
    ...
    if (m_in->read(priv + sizeof(uint32_t), size - sizeof(uint32_t)) !=
        (size - sizeof(uint32_t)))
      mxerror(boost::format(Y("Quicktime/MP4 reader: Could not read the stream "
...
void qtmp4_reader_c::handle_stss_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  m_in->skip(1 + 3);  // version & flags
  uint32_t count = m_in->read_uint32_be();
  ...
  mxdebug_if(m_debug_headers,
             boost::format("%1%Sync/keyframe table: %2% entries\n") %
                 space(level * 2 + 1) % count);
  if (m_debug_tables)
...
void qtmp4_reader_c::handle_stsz_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  m_in->skip(1 + 3);  // version & flags
  ...
  if (0 == sample_size) {
    ...
    for (i = 0; i < count; ++i) {
      ...
      if (sample.size >= 100 * 1024 * 1024) sample.size = 0;

      new_dmx->sample_table.push_back(sample);
    }

    mxdebug_if(m_debug_headers,
               boost::format("%1%Sample size table: %2% entries\n") %
                   space(level * 2 + 1) % count);
    if (m_debug_tables) {
      auto i = 0u;
    ...
    mxdebug_if(
        m_debug_headers,
        boost::format("%1%Sample size table; global sample size: %2%\n") %
            space(level * 2 + 1) % sample_size);
  }
}

void qtmp4_reader_c::handle_sttd_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  m_in->skip(1 + 3);  // version & flags
  uint32_t count = m_in->read_uint32_be();
  ...
  mxdebug_if(m_debug_headers,
             boost::format("%1%Sample duration table: %2% entries\n") %
                 space(level * 2 + 1) % count);
  if (m_debug_tables) {
    i = 0;
    for (auto const &durmap : new_dmx->durmap_table)
      mxdebug(boost::format("%1%%2%: number %3% duration %4%\n") %
              space((level + 1) * 2 + 1) % i++ % durmap.number %
              durmap.duration);
  }
}

void qtmp4_reader_c::handle_stts_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  m_in->skip(1 + 3);  // version & flags
  uint32_t count = m_in->read_uint32_be();
...
void qtmp4_reader_c::handle_edts_atom(qtmp4_demuxer_cptr &new_dmx,
                                      qt_atom_t parent, int level) {
  while (0 < parent.size) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();
...
void qtmp4_reader_c::handle_elst_atom(qtmp4_demuxer_cptr &new_dmx, qt_atom_t,
                                      int level) {
  ...
  mxdebug_if(m_debug_headers,
             boost::format("%1%Edit list table: %2% entries\n") %
                 space(level * 2 + 1) % count);
  if (!m_debug_tables) return;

  i = 0;
  for (auto const &editlist : new_dmx->editlist_table)
    mxdebug_if(m_debug_tables,
               boost::format("%1%%2%: segment duration %3% media time %4% "
                             "media rate %5%.%6%\n") %
                   space((level + 1) * 2 + 1) % i++ %
                   editlist.segment_duration % editlist.media_time %
                   editlist.media_rate_integer % editlist.media_rate_fraction);
}

void qtmp4_reader_c::handle_tkhd_atom(qtmp4_demuxer_cptr &new_dmx,
                                      qt_atom_t atom, int level) {
  tkhd_atom_t tkhd;
  ...
  mxdebug_if(
      m_debug_headers,
      boost::format(
          "%1%Track ID: %2% display width × height %3%.%4% × %5%.%6%\n") %
          space(level * 2 + 1) % new_dmx->container_id %
          (new_dmx->v_display_width_flt >> 16) %
          (new_dmx->v_display_width_flt & 0xffff) %
          (new_dmx->v_display_height_flt >> 16) %
          (new_dmx->v_display_height_flt & 0xffff));
}

void qtmp4_reader_c::handle_tref_atom(qtmp4_demuxer_cptr & /* new_dmx */,
                                      qt_atom_t parent, int level) {
  while (parent.size > 0) {
    qt_atom_t atom = read_atom();
    if ((atom.size > parent.size) || (12 > atom.size)) break;
    ...
    if (atom.fourcc == "chap")
      for (auto track_id : track_ids) m_chapter_track_ids[track_id] = true;
...
void qtmp4_reader_c::handle_trak_atom(qtmp4_demuxer_cptr &new_dmx,
                                      qt_atom_t parent, int level) {
  while (parent.size > 0) {
    qt_atom_t atom = read_atom();
    print_basic_atom_info();
...
file_status_e qtmp4_reader_c::read(generic_packetizer_c *ptzr, bool) {
  size_t dmx_idx;

  for (dmx_idx = 0; dmx_idx < m_demuxers.size(); ++dmx_idx) {
    qtmp4_demuxer_cptr &dmx = m_demuxers[dmx_idx];

    if ((-1 == dmx->ptzr) || (PTZR(dmx->ptzr) != ptzr)) continue;

    if (dmx->pos < dmx->m_index.size()) break;
  }

  if (m_demuxers.size() == dmx_idx) return flush_packetizers();
  ...
  if (dmx->is_video() && !dmx->pos &&
      dmx->codec.is(codec_c::type_e::V_MPEG4_P2) && dmx->esds_parsed &&
    ...
    memcpy(buffer->get_buffer(), dmx->esds.decoder_config->get_buffer(),
           dmx->esds.decoder_config->get_size());

  } else if (dmx->is_video() && dmx->codec.is(codec_c::type_e::V_PRORES) &&
             (index.size >= 8)) {
    m_in->skip(8);
    index.size -= 8;
  ...
  if (m_in->read(buffer->get_buffer() + buffer_offset, index.size) !=
      index.size) {
    mxwarn(boost::format(
               Y("Quicktime/MP4 reader: Could not read chunk number %1%/%2% "
                 "with size %3% from position %4%. Aborting.\n")) %
           dmx->pos % dmx->m_index.size() % index.size % index.file_pos);
    return flush_packetizers();
  }

  PTZR(dmx->ptzr)->process(new packet_t(
      buffer, index.timecode, index.duration,
      index.is_keyframe ? VFT_IFRAME : VFT_PFRAMEAUTOMATIC, VFT_NOBFRAME));
  ++dmx->pos;

  if (dmx->pos < dmx->m_index.size()) return FILE_STATUS_MOREDATA;

  return flush_packetizers();
}

memory_cptr qtmp4_reader_c::create_bitmap_info_header(qtmp4_demuxer_cptr &dmx,
                                                      const char *fourcc,
                                                      size_t extra_size,
  ...
  if (0 != extra_size) memcpy(bih + 1, extra_data, extra_size);

  return bih_p;
}

bool qtmp4_reader_c::create_audio_packetizer_ac3(qtmp4_demuxer_cptr &dmx) {
  auto buf = dmx->read_first_bytes(64);

  if (!buf || (-1 == dmx->m_ac3_header.find_in(buf))) {
    mxwarn_tid(
        m_ti.m_fname, dmx->id,
        Y("No AC-3 header found in first frame; track will be skipped.\n"));
    dmx->ok = false;

    return false;
  }

  dmx->ptzr = add_packetizer(new ac3_packetizer_c(
      this, m_ti, dmx->m_ac3_header.m_sample_rate, dmx->m_ac3_header.m_channels,
      dmx->m_ac3_header.m_bs_id, true));
  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));

  return true;
}

bool qtmp4_reader_c::create_audio_packetizer_alac(qtmp4_demuxer_cptr &dmx) {
  auto magic_cookie = memory_c::clone(
      dmx->stsd->get_buffer() + dmx->stsd_non_priv_struct_size + 12,
      dmx->stsd->get_size() - dmx->stsd_non_priv_struct_size - 12);
  dmx->ptzr = add_packetizer(new alac_packetizer_c(
      this, m_ti, magic_cookie, dmx->a_samplerate, dmx->a_channels));
  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));

  return true;
}

bool qtmp4_reader_c::create_audio_packetizer_dts(qtmp4_demuxer_cptr &dmx) {
  dmx->ptzr =
      add_packetizer(new dts_packetizer_c(this, m_ti, dmx->m_dts_header));
  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));

  return true;
}

void qtmp4_reader_c::create_video_packetizer_svq1(qtmp4_demuxer_cptr &dmx) {
  m_ti.m_private_data = create_bitmap_info_header(dmx, "SVQ1");
  dmx->ptzr = add_packetizer(new video_for_windows_packetizer_c(
      this, m_ti, 0.0, dmx->v_width, dmx->v_height));

  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_video_packetizer_mpeg4_p2(qtmp4_demuxer_cptr &dmx) {
  m_ti.m_private_data = create_bitmap_info_header(dmx, "DIVX");
  dmx->ptzr = add_packetizer(new mpeg4_p2_video_packetizer_c(
      this, m_ti, 0.0, dmx->v_width, dmx->v_height, false));

  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_video_packetizer_mpeg1_2(qtmp4_demuxer_cptr &dmx) {
  int version = (dmx->fourcc.value() & 0xff) - '0';
  dmx->ptzr = add_packetizer(new mpeg1_2_video_packetizer_c(
      this, m_ti, version, -1.0, dmx->v_width, dmx->v_height, 0, 0, false));

  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_video_packetizer_mpeg4_p10(
    qtmp4_demuxer_cptr &dmx) {
  m_ti.m_private_data = dmx->priv;
  dmx->ptzr = add_packetizer(new mpeg4_p10_video_packetizer_c(
      this, m_ti, dmx->fps, dmx->v_width, dmx->v_height));

  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_video_packetizer_mpegh_p2(qtmp4_demuxer_cptr &dmx) {
  m_ti.m_private_data = dmx->priv;
  dmx->ptzr = add_packetizer(new hevc_video_packetizer_c(
      this, m_ti, dmx->fps, dmx->v_width, dmx->v_height));

  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_video_packetizer_prores(qtmp4_demuxer_cptr &dmx) {
  m_ti.m_private_data = memory_c::clone(dmx->fourcc.str());
  dmx->ptzr = add_packetizer(new generic_video_packetizer_c(
      this, m_ti, MKV_V_PRORES, dmx->fps, dmx->v_width, dmx->v_height));

  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_video_packetizer_standard(qtmp4_demuxer_cptr &dmx) {
  m_ti.m_private_data = dmx->stsd;
  dmx->ptzr = add_packetizer(new quicktime_video_packetizer_c(
      this, m_ti, dmx->v_width, dmx->v_height));

  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_audio_packetizer_aac(qtmp4_demuxer_cptr &dmx) {
  m_ti.m_private_data = dmx->esds.decoder_config;
  dmx->ptzr = add_packetizer(
      new aac_packetizer_c(this, m_ti, dmx->a_aac_profile, dmx->a_samplerate,
                           dmx->a_channels, true));

  if (dmx->a_aac_is_sbr)
    PTZR(dmx->ptzr)->set_audio_output_sampling_freq(
        dmx->a_aac_output_sample_rate);

  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_audio_packetizer_mp3(qtmp4_demuxer_cptr &dmx) {
  dmx->ptzr = add_packetizer(new mp3_packetizer_c(this, m_ti, dmx->a_samplerate,
                                                  dmx->a_channels, true));
  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_audio_packetizer_pcm(qtmp4_demuxer_cptr &dmx) {
  dmx->ptzr = add_packetizer(new pcm_packetizer_c(
      this, m_ti, static_cast<int32_t>(dmx->a_samplerate), dmx->a_channels,
      dmx->a_bitdepth, dmx->m_pcm_format));
  show_packetizer_info(dmx->id, PTZR(dmx->ptzr));
}

void qtmp4_reader_c::create_audio_packetizer_passthrough(
    qtmp4_demuxer_cptr &dmx) {
...
void qtmp4_reader_c::create_subtitles_packetizer_vobsub(
    qtmp4_demuxer_cptr &dmx) {
  std::string palette;
  auto format = [](int v) {
    return (boost::format("%|1$02x|") % std::min(std::max(v, 0), 255)).str();
  };
  ...
  auto idx_str =
      (boost::format(
           ...
           "hh:mm:ss:ms)\n"
           "# Note: Not effective in DirectVobSub, use \"delay: ... \" "
           "instead.\n"
           "time offset: 0\n"
           "\n"
           ...
           "000000\n") %
       dmx->v_width % dmx->v_height % palette)
          .str();
...
void qtmp4_reader_c::create_packetizer(int64_t tid) {
  ...
  if (!dmx || !dmx->ok ||
      !demuxing_requested(dmx->type, dmx->id, dmx->language) ||
      (-1 != dmx->ptzr))
    return;
  ...
  if (dmx->is_video()) {
    ...
    else if (dmx->codec.is(codec_c::type_e::A_MP2) ||
             dmx->codec.is(codec_c::type_e::A_MP3))
  ...
  if (packetizer_ok && (-1 == m_main_dmx)) m_main_dmx = i;
}
...
void qtmp4_reader_c::create_packetizers() {
  ...
  for (i = 0; i < m_demuxers.size(); ++i) create_packetizer(m_demuxers[i]->id);
}

int qtmp4_reader_c::get_progress() {
  if (-1 == m_main_dmx) return 100;

  qtmp4_demuxer_cptr &dmx = m_demuxers[m_main_dmx];
  unsigned int max_chunks = (0 == dmx->sample_size) ? dmx->sample_table.size()
                                                    : dmx->chunk_table.size();
...
void qtmp4_reader_c::identify() {
  ...
  for (i = 0; i < m_demuxers.size(); ++i) {
    ...
    if (dmx->is_video())
      info.add(mtx::id::pixel_dimensions,
               boost::format("%1%x%2%") % dmx->v_width % dmx->v_height);

    else if (dmx->is_audio()) {
      info.add(mtx::id::audio_channels, dmx->a_channels);
      info.add(mtx::id::audio_sampling_frequency,
               static_cast<uint64_t>(dmx->a_samplerate));
      info.add(mtx::id::audio_bits_per_sample, dmx->a_bitdepth);
    }

    id_result_track(dmx->id,
                    dmx->is_video()
                        ? ID_RESULT_TRACK_VIDEO
                        : dmx->is_audio()
                              ? ID_RESULT_TRACK_AUDIO
                              : dmx->is_subtitles() ? ID_RESULT_TRACK_SUBTITLES
                                                    : ID_RESULT_TRACK_UNKNOWN,
                    dmx->codec.get_name(dmx->fourcc.description()), info.get());
  }

  if (m_chapters) id_result_chapters(count_chapter_atoms(*m_chapters));
}

void qtmp4_reader_c::add_available_track_ids() {
  unsigned int i;
...
std::string qtmp4_reader_c::decode_and_verify_language(
    uint16_t coded_language) {
  std::string language;
  int i;

  for (i = 0; 3 > i; ++i)
    language +=
        static_cast<char>(((coded_language >> ((2 - i) * 5)) & 0x1f) + 0x60);

  int idx = map_to_iso639_2_code(balg::to_lower_copy(language));
  if (-1 != idx) return g_iso639_languages[idx].iso639_2_code;

  return "";
}

void qtmp4_reader_c::recode_chapter_entries(
    std::vector<qtmp4_chapter_entry_t> &entries) {
  if (g_identifying) {
    for (auto &entry : entries) entry.m_name = empty_string;
    return;
  }

  std::string charset =
      m_ti.m_chapter_charset.empty() ? "UTF-8" : m_ti.m_chapter_charset;
  charset_converter_cptr converter =
      charset_converter_c::init(m_ti.m_chapter_charset);
  ...
  for (auto &entry : entries) entry.m_name = converter->utf8(entry.m_name);

  converter->enable_byte_order_marker_detection(false);
}

void qtmp4_reader_c::detect_interleaving() {
  std::list<qtmp4_demuxer_cptr> demuxers_to_read;
  boost::remove_copy_if(
      m_demuxers, std::back_inserter(demuxers_to_read),
      [&](const qtmp4_demuxer_cptr &dmx) {
        return !(dmx->ok && (dmx->is_audio() || dmx->is_video()) &&
                 demuxing_requested(dmx->type, dmx->id, dmx->language) &&
                 (dmx->sample_table.size() > 1));
      });

  if (demuxers_to_read.size() < 2) {
    mxdebug_if(
        m_debug_interleaving,
        boost::format(
            "Interleaving: Not enough tracks to care about interleaving.\n"));
    return;
  }

  auto cmp = [](const qt_sample_t &s1, const qt_sample_t &s2) -> uint64_t {
  ...
  for (auto &dmx : demuxers_to_read) {
    ...
    mxdebug_if(
        m_debug_interleaving,
        boost::format(
            "Interleaving: Track id %1% min %2% max %3% gradient %4%\n") %
            dmx->id % min % max % gradients.back());
  }

  double badness =
      *boost::max_element(gradients) - *boost::min_element(gradients);
  mxdebug_if(
      m_debug_interleaving,
      boost::format("Interleaving: Badness: %1% (%2%)\n") % badness %
          (MAX_INTERLEAVING_BADNESS < badness ? "badly interleaved" : "ok"));

  if (MAX_INTERLEAVING_BADNESS < badness) m_in->enable_buffering(false);
}

// ----------------------------------------------------------------------

void qtmp4_demuxer_c::calculate_fps() {
  fps = 0.0;
  ...
  if ((1 == durmap_table.size()) && (0 != durmap_table[0].duration) &&
    ...
    fps = static_cast<double>(time_scale) /
          static_cast<double>(durmap_table[0].duration);
    mxdebug_if(m_debug_fps,
               boost::format("calculate_fps: case 1: %1%\n") % fps);

  } else if (!sample_table.empty()) {
    std::map<int64_t, int> duration_map;

    std::accumulate(sample_table.begin() + 1, sample_table.end(),
                    sample_table[0],
                    [&](qt_sample_t &previous_sample,
                        qt_sample_t &current_sample) -> qt_sample_t {
                      duration_map[current_sample.pts - previous_sample.pts]++;
                      return current_sample;
                    });

    auto most_common = std::accumulate(
        duration_map.begin(), duration_map.end(),
        std::pair<int64_t, int>(*duration_map.begin()),
        [](std::pair<int64_t, int> &a, std::pair<int64_t, int> e) {
          return e.second > a.second ? e : a;
        });
    ...
    mxdebug_if(m_debug_fps,
               boost::format(
                   "calculate_fps: case 2: most_common %1% = %2%, fps %3%\n") %
                   most_common.first % most_common.second % fps);
  }
}

int64_t qtmp4_demuxer_c::to_nsecs(int64_t value) {
  int i;
...
void qtmp4_demuxer_c::calculate_timecodes_constant_sample_size() {
  auto frame = 0u;
  for (auto &chunk : chunk_table) {
    timecodes.push_back(
        to_nsecs(static_cast<uint64_t>(chunk.samples) * duration) +
        constant_editlist_offset_ns);
    durations.push_back(to_nsecs(static_cast<uint64_t>(chunk.size) * duration));
    frame_indices.push_back(frame++);
  }
}

void qtmp4_demuxer_c::calculate_timecodes_variable_sample_size() {
  ...
  int64_t v_dts_offset = (is_avc || is_hevc) && num_frame_offsets
                             ? to_nsecs(frame_offset_table[0])
                             : 0;

  std::vector<int64_t> timecodes_before_offsets;

  for (unsigned int frame = 0, num_samples = sample_table.size();
       num_samples > frame; ++frame) {
    ...
    if (0 < num_edits) {
      ...
      while (((num_edits - 1) > editlist_pos) &&
  ...
  for (unsigned int frame = 0, num_timecodes_before_offsets =
                                   timecodes_before_offsets.size();
       num_timecodes_before_offsets > (frame + 1); ++frame) {
    int64_t diff =
        timecodes_before_offsets[frame + 1] - timecodes_before_offsets[frame];
  ...
  if (num_good_frames) {
    ...
    for (auto &duration : durations)
      if (!duration) duration = avg_duration;
  }
}

void qtmp4_demuxer_c::calculate_timecodes() {
  if (m_timecodes_calculated) return;
...
void qtmp4_demuxer_c::adjust_timecodes(int64_t delta) {
  for (auto &timecode : timecodes) timecode += delta;

  for (auto &index : m_index) index.timecode += delta;
}

int64_t qtmp4_demuxer_c::min_timecode() const {
  return timecodes.empty() ? 0 : *brng::min_element(timecodes);
}

bool qtmp4_demuxer_c::update_tables() {
  if (m_tables_updated) return true;
  ...
  while (i > 0) {
    ...
    if (chunk_table.size() <= last) break;
  }
  ...
  if (sample_table.empty()) {
    ...
    if ((1 == durmap_table.size()) ||
        ((2 == durmap_table.size()) && (1 == durmap_table[1].number)))
      duration = durmap_table[0].duration;
    else
      mxerror(
          Y("Quicktime/MP4 reader: Constant sample size & variable duration "
            "not yet supported. Contact the author if you have such a sample "
            "file.\n"));
...
void qtmp4_demuxer_c::update_editlist_table() {
  if (editlist_table.empty()) return;
  ...
  if ((editlist_table.size() == 1) && (0 == editlist_table[0].media_time)) {
    mxdebug_if(m_debug_editlists,
               boost::format("Track ID %1%: Edit list analysis: type 1: one "
                             "entry, zero time\n") %
                   id);
    simple_editlist_type = 1;

  } else if ((editlist_table.size() == 1) &&
             (0 < editlist_table[0].media_time)) {
    mxdebug_if(m_debug_editlists,
               boost::format("Track ID %1%: Edit list analysis: type 2: one "
                             "entry, positive time, %2%\n") %
                   id %
                   (frame_offset_table.empty()
                        ? "no frame offset table"
                        : frame_offset_table[0] == editlist_table[0].media_time
                              ? "same as first frame offset"
                              : "different from first frame offset"));
    simple_editlist_type = 2;
    raw_offset = editlist_table[0].media_time;
    constant_editlist_offset_ns =
        (-editlist_table[0].media_time +
         (frame_offset_table.empty() ? 0 : frame_offset_table[0])) *
        1000000000ll / time_scale;

  } else if ((editlist_table.size() == 2) &&
             (-1 == editlist_table[0].media_time)) {
    mxdebug_if(m_debug_editlists,
               boost::format("Track ID %1%: Edit list analysis: type 3: two "
                             "entries; first with time == -1\n") %
                   id);
    simple_editlist_type = 3;
    raw_offset = editlist_table[0].segment_duration;
    constant_editlist_offset_ns =
        (editlist_table[0].segment_duration * 1000000000ll /
         global_time_scale) -
        ((frame_offset_table.empty() ? 0 : frame_offset_table[0]) *
         1000000000ll / time_scale);
    ...
    for (auto &edit : editlist_table)
      output +=
          (boost::format(" <d:%1% t:%2% r:%3%.%4%>") % edit.segment_duration %
           edit.media_time % edit.media_rate_integer % edit.media_rate_fraction)
              .str();
    mxdebug(boost::format(
                "Track ID %1%: Edit list analysis: type 0; %2% entries:%3%\n") %
  ...
  if (simple_editlist_type) {
    ...
    mxdebug_if(
        m_debug_editlists,
        boost::format("Track ID %1%: Simple edit list type detected. Offset in "
                      "%5%'s time scale: %2%; as a timecode: %3%; time scale "
                      "%4%\n") %
            id % raw_offset % format_timestamp(constant_editlist_offset_ns) %
            (offset_in_global_time_scale ? global_time_scale : time_scale) %
            (offset_in_global_time_scale ? "file" : "track"));
  ...
  // int64_t pts_offset       = frame_offset_table.empty() ? 0 :
  // frame_offset_table[0];
  // if (('v' == type) && codec.is(codec_c::type_e::V_MPEG4_P10) &&
  // !frame_offset_table.empty())
  //   pts_offset = frame_offset_table[0];

  mxdebug_if(m_debug_tables,
             boost::format("Updating edit list table for track %1%\n") % id);

  for (i = 0; editlist_table.size() > i; ++i) {
    auto &el = editlist_table[i];
    int64_t pts =
        el.media_time - (i < num_frame_offsets ? frame_offset_table[i] : 0);
    auto sample = 0u;
    el.start_frame = frame;

    // find start sample
    for (; sample_table.size() > sample; ++sample)
      if (pts <= sample_table[sample].pts) break;

    el.start_sample = sample;
    el.pts_offset =
        (e_pts * time_scale) / global_time_scale - sample_table[sample].pts;
    ...
    mxdebug_if(m_debug_tables,
               boost::format("  %1%: pts: %2%  1st_sample: %3%  frames: %4% "
                             "(%|5$5.3f|s)  pts_offset: %6%\n") %
                   i % el.media_time % el.start_sample % el.frames %
                   (static_cast<double>(el.segment_duration) / time_scale) %
                   el.pts_offset);
  }
}

void qtmp4_demuxer_c::build_index() {
  if (sample_size != 0)
...
void qtmp4_demuxer_c::build_index_constant_sample_size_mode() {
  size_t keyframe_table_idx = 0;
  size_t keyframe_table_size = keyframe_table.size();

  auto is_audio = 'a' == type;
  auto sound_stsd_atom = reinterpret_cast<sound_v1_stsd_atom_t *>(
      is_audio && stsd ? stsd->get_buffer() : nullptr);
  auto v0_sample_size =
      sound_stsd_atom ? get_uint16_be(&sound_stsd_atom->v0.sample_size) : 0;
  auto v0_audio_version =
      sound_stsd_atom ? get_uint16_be(&sound_stsd_atom->v0.version) : 0;
  auto v1_bytes_per_frame =
      1 == v0_audio_version
          ? get_uint32_be(&sound_stsd_atom->v1.bytes_per_frame)
          : 0;
  auto v1_samples_per_packet =
      1 == v0_audio_version
          ? get_uint32_be(&sound_stsd_atom->v1.samples_per_packet)
          : 0;
  ...
  for (frame_idx = 0; frame_idx < chunk_table.size(); ++frame_idx) {
    ...
    if (keyframe_table.empty())
      is_keyframe = true;
    else if ((keyframe_table_idx < keyframe_table_size) &&
             ((frame_idx + 1) == keyframe_table[keyframe_table_idx])) {
      is_keyframe = true;
      ++keyframe_table_idx;
    }

    m_index.push_back(qt_index_t(chunk_table[frame_idx].pos, frame_size,
                                 timecodes[frame_idx], durations[frame_idx],
                                 is_keyframe));
  }
}

void qtmp4_demuxer_c::build_index_chunk_mode() {
  size_t keyframe_table_idx = 0;
  size_t keyframe_table_size = keyframe_table.size();
  ...
  for (frame_idx = 0; frame_idx < frame_indices.size(); ++frame_idx) {
    ...
    m_index.push_back(qt_index_t(
        sample_table[act_frame_idx].pos, sample_table[act_frame_idx].size,
        timecodes[frame_idx], durations[frame_idx], is_keyframe));
  }
}

void qtmp4_demuxer_c::mark_open_gop_random_access_points_as_key_frames() {
  // Mark samples indicated by the 'rap ' sample group to be key
  // frames, too.
  auto table_itr = sample_to_group_tables.find(fourcc_c{"rap "}.value());
  if (table_itr == sample_to_group_tables.end()) return;
  ...
  for (auto const &s2g : table_itr->second) {
    if (s2g.group_description_index &&
        ((s2g.group_description_index - 1) < num_random_access_points)) {
      for (auto end = std::min<size_t>(current_sample + s2g.sample_count,
                                       num_index_entries);
           current_sample < end; ++current_sample)
        m_index[current_sample].is_keyframe = true;

    } else
      current_sample += s2g.sample_count;

    if (current_sample >= num_index_entries) return;
  }
}

memory_cptr qtmp4_demuxer_c::read_first_bytes(int num_bytes) {
  if (!update_tables()) return memory_cptr{};
  ...
  while ((0 < num_bytes) && (idx_pos < m_index.size())) {
    ...
    if (m_reader.m_in->read(buf->get_buffer() + buf_pos, num_bytes_to_read) <
        num_bytes_to_read)
      return memory_cptr{};
...
bool qtmp4_demuxer_c::is_audio() const { return 'a' == type; }

bool qtmp4_demuxer_c::is_video() const { return 'v' == type; }

bool qtmp4_demuxer_c::is_subtitles() const { return 's' == type; }

bool qtmp4_demuxer_c::is_chapters() const { return 'C' == type; }

bool qtmp4_demuxer_c::is_unknown() const {
  return !is_audio() && !is_video() && !is_subtitles() && !is_chapters();
}

void qtmp4_demuxer_c::set_packetizer_display_dimensions() {
  ...
  if ((display_width != 0) && (display_height != 0) &&
      ((v_width != display_width) || (v_height != display_height)))
    m_reader.m_reader_packetizers[ptzr]->set_video_display_dimensions(
        display_width, display_height, OPTION_SOURCE_CONTAINER);
}

void qtmp4_demuxer_c::handle_stsd_atom(uint64_t atom_size, int level) {
  if (is_audio()) {
    handle_audio_stsd_atom(atom_size, level);
    if ((0 < stsd_non_priv_struct_size) &&
        (stsd_non_priv_struct_size < atom_size))
...
void qtmp4_demuxer_c::handle_audio_stsd_atom(uint64_t atom_size, int level) {
  auto priv = stsd->get_buffer();
  auto size = stsd->get_size();

  if (sizeof(sound_v0_stsd_atom_t) > atom_size)
    mxerror(boost::format(Y("Quicktime/MP4 reader: Could not read the sound "
                            "description atom for track ID %1%.\n")) %
            id);
  ...
  if (fourcc)
    mxwarn(boost::format(
               Y("Quicktime/MP4 reader: Track ID %1% has more than one FourCC. "
                 "Only using the first one (%2%) and not this one (%3%).\n")) %
           id % fourcc.description() %
           fourcc_c{sv1_stsd.v0.base.fourcc}.description());
  else
  ...
  mxdebug_if(
      m_debug_headers,
      boost::format("%1%[v0] FourCC: %2% channels: %3% sample size: %4% "
                    "compression id: %5% sample rate: %6% version: %7%") %
          space(level * 2 + 1) %
          fourcc_c{
              reinterpret_cast<const unsigned char *>(sv1_stsd.v0.base.fourcc)}
              .description() %
          a_channels % a_bitdepth % get_uint16_be(&sv1_stsd.v0.compression_id) %
          a_samplerate % get_uint16_be(&sv1_stsd.v0.version));
  ...
  if (0 == version)
    ...
    if (sizeof(sound_v1_stsd_atom_t) > size)
      mxerror(
          boost::format(Y("Quicktime/MP4 reader: Could not read the extended "
                          "sound description atom for track ID %1%.\n")) %
          id);
    ...
    if (m_debug_headers)
      mxinfo(boost::format(" [v1] samples per packet: %1% bytes per packet: "
                           "%2% bytes per frame: %3% bytes_per_sample: %4%") %
    ...
    if (m_debug_headers)
      mxinfo(
          boost::format(" [v2] struct size: %1% sample rate: %2% channels: %3% "
                        "const1: 0x%|4$08x| bits per channel: %5% flags: %6% "
                        "bytes per frame: %7% samples per frame: %8%") %
          get_uint32_be(&sv2_stsd.v2.v2_struct_size) % a_samplerate %
          a_channels % get_uint32_be(&sv2_stsd.v2.const1) % a_bitdepth %
          get_uint32_be(&sv2_stsd.v2.flags) %
          get_uint32_be(&sv2_stsd.v2.bytes_per_frame) %
          get_uint32_be(&sv2_stsd.v2.samples_per_frame));
  }

  if (m_debug_headers) mxinfo("\n");
}

void qtmp4_demuxer_c::handle_video_stsd_atom(uint64_t atom_size, int level) {
  stsd_non_priv_struct_size = sizeof(video_stsd_atom_t);

  if (sizeof(video_stsd_atom_t) > atom_size)
    mxerror(boost::format(Y("Quicktime/MP4 reader: Could not read the video "
                            "description atom for track ID %1%.\n")) %
            id);
  ...
  mxdebug_if(
      m_debug_headers,
      boost::format("%1%FourCC: %2%, width: %3%, height: %4%, depth: %5%\n") %
          space(level * 2 + 1) % fourcc_c{v_stsd.base.fourcc}.description() %
...
void qtmp4_demuxer_c::handle_subtitles_stsd_atom(uint64_t atom_size,
                                                 int level) {
  auto priv = stsd->get_buffer();
  auto size = stsd->get_size();

  if (sizeof(base_stsd_atom_t) > atom_size) return;
...
void qtmp4_demuxer_c::parse_audio_header_priv_atoms(uint64_t atom_size,
                                                    int level) {
  ...
  try {
    while (!mio.eof() && (mio.getFilePointer() < (size - 8))) {
      ...
      mxdebug_if(
          m_debug_headers,
          boost::format("%1%Audio private data size: %2%, type: '%3%'\n") %
      ...
      if (!esds_parsed) {
        ...
        if (esds_parsed &&
            codec_c::look_up_object_type_id(esds.object_type_id)
                .is(codec_c::type_e::A_AAC)) {
          int profile, sample_rate, channels, output_sample_rate;
          bool aac_is_sbr;

          if (!esds.decoder_config || (2 > esds.decoder_config->get_size()))
            mxwarn(boost::format(Y("Track %1%: AAC found, but decoder config "
                                   "data has length %2%.\n")) %
                   id %
                   (esds.decoder_config ? esds.decoder_config->get_size() : 0));

          else if (!aac::parse_audio_specific_config(
                       esds.decoder_config->get_buffer(),
                       esds.decoder_config->get_size(), profile, channels,
                       sample_rate, output_sample_rate, aac_is_sbr))
            mxwarn(
                boost::format(Y(
                    "Track %1%: The AAC information could not be parsed.\n")) %
                id);

          else {
            mxdebug_if(
                m_debug_headers,
                boost::format(" AAC: profile: %1%, sample_rate: %2%, channels: "
                              "%3%, output_sample_rate: %4%, sbr: %5%\n") %
                    profile % sample_rate % channels % output_sample_rate %
                    aac_is_sbr);
            if (aac_is_sbr) profile = AAC_PROFILE_SBR;
...
void qtmp4_demuxer_c::parse_video_header_priv_atoms(uint64_t atom_size,
                                                    int level) {
  auto mem = stsd->get_buffer() + stsd_non_priv_struct_size;
  auto size = atom_size - stsd_non_priv_struct_size;

  if (!codec.is(codec_c::type_e::V_MPEG4_P10) &&
      !codec.is(codec_c::type_e::V_MPEGH_P2) && size && !fourcc.equiv("mp4v") &&
      !fourcc.equiv("xvid")) {
  ...
  try {
    while (!mio.eof() && (mio.getFilePointer() < size)) {
      ...
      mxdebug_if(
          m_debug_headers,
          boost::format("%1%Video private data size: %2%, type: '%3%'\n") %
              space((level + 1) * 2 + 1) % atom.size % atom.fourcc);
...
void qtmp4_demuxer_c::parse_subtitles_header_priv_atoms(uint64_t atom_size,
                                                        int level) {
  auto mem = stsd->get_buffer() + stsd_non_priv_struct_size;
  auto size = atom_size - stsd_non_priv_struct_size;
  ...
  try {
    while (!mio.eof() && (mio.getFilePointer() < size)) {
      ...
      mxdebug_if(
          m_debug_headers,
          boost::format("%1%Subtitles private data size: %2%, type: '%3%'\n") %
              space((level + 1) * 2 + 1) % atom.size % atom.fourcc);
  ...
  mxdebug_if(m_debug_headers,
             boost::format("%1%Decoder config data at end of parsing: %2%\n") %
                 space((level + 1) * 2 + 1) %
                 (esds.decoder_config ? esds.decoder_config->get_size() : 0));
}

bool qtmp4_demuxer_c::parse_esds_atom(mm_mem_io_c &memio, int level) {
  int lsp = (level + 1) * 2;
  ...
  mxdebug_if(m_debug_headers,
             boost::format("%1%esds: version: %2%, flags: %3%\n") %
                 space(lsp + 1) % static_cast<unsigned int>(esds.version) %
                 esds.flags);
  ...
  if (MP4DT_ES == tag) {
    ...
    mxdebug_if(
        m_debug_headers,
        boost::format("%1%esds: id: %2%, stream priority: %3%, len: %4%\n") %
            space(lsp + 1) % static_cast<unsigned int>(esds.esid) %
            static_cast<unsigned int>(esds.stream_priority) % len);

  } else {
    esds.esid = memio.read_uint16_be();
    mxdebug_if(m_debug_headers, boost::format("%1%esds: id: %2%\n") %
                                    space(lsp + 1) %
                                    static_cast<unsigned int>(esds.esid));
  }
  ...
  if (MP4DT_DEC_CONFIG != tag) {
    mxdebug_if(m_debug_headers,
               boost::format(
                   "%1%tag is not DEC_CONFIG (0x%|2$02x|) but 0x%|3$02x|.\n") %
  ...
  mxdebug_if(
      m_debug_headers,
      boost::format(
          "%1%esds: decoder config descriptor, len: %2%, object_type_id: %3%, "
          "stream_type: 0x%|4$2x|, buffer_size_db: %5%, max_bitrate: "
          "%|6$.3f|kbit/s, avg_bitrate: %|7$.3f|kbit/s\n") %
          space(lsp + 1) % len %
          static_cast<unsigned int>(esds.object_type_id) %
          static_cast<unsigned int>(esds.stream_type) % esds.buffer_size_db %
          (esds.max_bitrate / 1000.0) % (esds.avg_bitrate / 1000.0));

  tag = memio.read_uint8();
  if (MP4DT_DEC_SPECIFIC == tag) {
    len = memio.read_mp4_descriptor_len();
    if (!len) throw mtx::input::header_parsing_x();
    ...
    if (m_debug_headers) {
      mxdebug(
          boost::format("%1%esds: decoder specific descriptor, len: %2%\n") %
          space(lsp + 1) % len);
      mxdebug(boost::format("%1%esds: dumping decoder specific descriptor\n") %
              space(lsp + 3));
      debugging_c::hexdump(esds.decoder_config->get_buffer(),
                           esds.decoder_config->get_size());
    }
    ...
    mxdebug_if(
        m_debug_headers,
        boost::format(
            "%1%tag is not DEC_SPECIFIC (0x%|2$02x|) but 0x%|3$02x|.\n") %
  ...
  if (MP4DT_SL_CONFIG == tag) {
    ...
    mxdebug_if(
        m_debug_headers,
        boost::format("%1%esds: sync layer config descriptor, len: %2%\n") %
            space(lsp + 1) % len);

  } else
    mxdebug_if(m_debug_headers,
               boost::format(
                   "%1%tag is not SL_CONFIG (0x%|2$02x|) but 0x%|3$02x|.\n") %
                   space(lsp + 1) % MP4DT_SL_CONFIG % tag);

  return true;
}

void qtmp4_demuxer_c::derive_track_params_from_ac3_audio_bitstream() {
  auto buf = read_first_bytes(64);
  auto header = ac3::frame_c{};

  if (!buf || (-1 == header.find_in(buf))) return;

  a_channels = header.m_channels;
  a_samplerate = header.m_sample_rate;
}

void qtmp4_demuxer_c::derive_track_params_from_dts_audio_bitstream() {
  auto buf = read_first_bytes(16384);
  auto header = mtx::dts::header_t{};

  if (!buf || (-1 == mtx::dts::find_header(buf->get_buffer(), buf->get_size(),
                                           header, false)))
    return;
...
void qtmp4_demuxer_c::derive_track_params_from_mp3_audio_bitstream() {
  auto buf = read_first_bytes(64);
  if (!buf) return;
...
bool qtmp4_demuxer_c::verify_audio_parameters() {
  ...
  if ((0 == a_channels) || (0.0 == a_samplerate)) {
    mxwarn(boost::format(Y("Quicktime/MP4 reader: Track %1% is missing some "
                           "data. Broken header atoms?\n")) %
           id);
    return false;
  }

  if (fourcc.equiv("MP4A") && !verify_mp4a_audio_parameters()) return false;

  if (codec.is(codec_c::type_e::A_ALAC)) return verify_alac_audio_parameters();

  if (codec.is(codec_c::type_e::A_DTS)) return verify_dts_audio_parameters();

  return true;
}

bool qtmp4_demuxer_c::verify_alac_audio_parameters() {
  if (!stsd || (stsd->get_size() < (stsd_non_priv_struct_size + 12 +
                                    sizeof(alac::codec_config_t)))) {
    mxwarn(boost::format(Y("Quicktime/MP4 reader: Track %1% is missing some "
                           "data. Broken header atoms?\n")) %
           id);
    return false;
...
bool qtmp4_demuxer_c::verify_dts_audio_parameters() {
  auto buf = read_first_bytes(16384);

  if (!buf || (-1 == mtx::dts::find_header(buf->get_buffer(), buf->get_size(),
                                           m_dts_header, false))) {
    mxwarn_tid(
        m_reader.m_ti.m_fname, id,
        Y("No DTS header found in first frames; track will be skipped.\n"));
    return false;
  }
...
bool qtmp4_demuxer_c::verify_mp4a_audio_parameters() {
  auto cdc = codec_c::look_up_object_type_id(esds.object_type_id);
  if (!cdc.is(codec_c::type_e::A_AAC) && !cdc.is(codec_c::type_e::A_DTS) &&
      !cdc.is(codec_c::type_e::A_MP2) && !cdc.is(codec_c::type_e::A_MP3)) {
    mxwarn(boost::format(Y("Quicktime/MP4 reader: The audio track %1% is using "
                           "an unsupported 'object type id' of %2% in the "
                           "'esds' atom. Skipping this track.\n")) %
           id % static_cast<unsigned int>(esds.object_type_id));
    return false;
  }

  if (cdc.is(codec_c::type_e::A_AAC) &&
      (!esds.decoder_config || !a_aac_config_parsed)) {
    mxwarn(boost::format(
               Y("Quicktime/MP4 reader: The AAC track %1% is missing the esds "
                 "atom/the decoder config. Skipping this track.\n")) %
           id);
    return false;
  }
...
bool qtmp4_demuxer_c::verify_video_parameters() {
  ...
  if (fourcc.equiv("mp4v")) return verify_mp4v_video_parameters();
...
bool qtmp4_demuxer_c::verify_avc_video_parameters() {
  if (!priv || (4 > priv->get_size())) {
    mxwarn(
        boost::format(Y("Quicktime/MP4 reader: MPEG4 part 10/AVC track %1% is "
                        "missing its decoder config. Skipping this track.\n")) %
        id);
    return false;
  }
...
bool qtmp4_demuxer_c::verify_hevc_video_parameters() {
  if (!priv || (23 > priv->get_size())) {
    mxwarn(
        boost::format(Y("Quicktime/MP4 reader: MPEGH part 2/HEVC track %1% is "
                        "missing its decoder config. Skipping this track.\n")) %
        id);
    return false;
  }
...
bool qtmp4_demuxer_c::verify_mp4v_video_parameters() {
  if (!esds_parsed) {
    mxwarn(boost::format(Y("Quicktime/MP4 reader: The video track %1% is "
                           "missing the ESDS atom. Skipping this track.\n")) %
           id);
    return false;
  }
  ...
  if (codec.is(codec_c::type_e::V_MPEG4_P2) && !esds.decoder_config) {
    ...
    mxwarn(boost::format(
               Y("Quicktime/MP4 reader: MPEG4 track %1% is missing the esds "
                 "atom/the decoder config. Skipping this track.\n")) %
           id);
    return false;
  }
...
bool qtmp4_demuxer_c::verify_subtitles_parameters() {
...
bool qtmp4_demuxer_c::verify_vobsub_subtitles_parameters() {
...
void qtmp4_demuxer_c::determine_codec() {
