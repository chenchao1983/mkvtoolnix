...
bool parse_number_as_rational(std::string const &string,
                              int64_rational_c &value);

template <typename StrT, typename ValueT>
bool parse_number(StrT const &string, ValueT &value) {
  try {
    mtx::conversion::unsigned_checker<
        std::is_unsigned<ValueT>::value>::do_check(string);
...
template <typename StrT>
bool parse_number(StrT const &string, int64_rational_c &value) {
  return parse_number_as_rational(string, value);
}

template <typename StrT>
bool parse_number(StrT const &string, double &value) {
  int64_rational_c rational_value;
  if (!parse_number(string, rational_value)) return false;
...
template <typename StrT>
bool parse_number(StrT const &string, float &value) {
  int64_rational_c rational_value;
  if (!parse_number(string, rational_value)) return false;
...
bool parse_property_to_struct(const std::string &s,
                              std::map<int64_t, StructType> &property_list) {
  int64_t id = 0;
  auto v = split(s, ":");
  if ((v.size() != 2) || !parse_number(v[0], id)) return false;

  v = split(v[1], ",");
  if (v.size() != StructType::num_properties) return false;

  std::vector<DataType> data;
  for (int i = 0; i < StructType::num_properties; ++i) {
    DataType value;
    if (!parse_number(v[i], value)) return false;
...
bool parse_property_to_value(const std::string &s,
  ...
  DataType value;
  if (!parse_number(v[1], value)) return false;
...
extern bool parse_timecode(const std::string &s, int64_t &timecode,
                           bool allow_negative = false);
extern bool parse_timecode(const std::string &s, timestamp_c &timecode,
                           bool allow_negative = false);
